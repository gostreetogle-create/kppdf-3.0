        ...
        <div class="editor__canvas-wrapper">
          ...
          >
                  ...
                  <table class="editor__table">
                    ...
                    <tbody>
                      <tr *ngFor="let item of items(); trackBy: trackByItem; let i = index">
                        ...
                        <td>
                          <div class="editor__table-photo" (click)="setItemPhoto(i)">
            ...
            <div class="editor__add-fab">
              ...
              />
              <div class="editor__add-menu" *ngIf="showAddMenu" (click)="showAddMenu = false">
...
export class DocumentEditorComponent implements OnInit {
  ...
  private loadQuotation(id: string): void {
    this.crudApi.getById<any>('/directories/quotations', id)
      .pipe(
        tap({
          next: (data) => {
  ...
  private loadTemplates(): void {
    this.crudApi.list<any>('/document-templates', { all: true })
      .pipe(
        tap({
          next: (res) => {
            this.templates.set(res.data || []);
            // If no template loaded yet and we have templates, apply default
            if (!this.activeTemplateId() && (res.data || []).length > 0) {
              const defaultTmpl = res.data.find((t: any) => t.isDefault && t.docType === 'quotation');
              if (defaultTmpl) {
  ...
  save(): void {
    ...
    if (this.isNew()) {
      this.crudApi.create('/directories/quotations', data)
        .pipe(
          tap({
            next: (result: any) => {
              ...
              this.router.navigate(['/documents', result._id], { replaceUrl: true });
            },
            error: (err) => {
  ...
  setItemPhoto(index: number): void {
  ...
  editBlockSettings(index: number): void {
    ...
    if (block.type === 'separator') {
      const padding = prompt('Отступ сверху (px):', String(block.settings.paddingTop));
      if (padding !== null) {
        this.blocks.update(blocks => {
          const updated = [...blocks];
          updated[index] = {
            ...updated[index],
            settings: { ...updated[index].settings, paddingTop: parseInt(padding) || 4 },
          };
          return updated;
        });
      }
  ...
  setBackgroundImage(): void {
    const url = prompt('Введите URL фонового изображения:');
    if (!url) return;

    // If no active template, create one named after current document
    ...
    if (activeId) {
      ...
      this.crudApi.create('/document-templates', {
        name: 'Фон для КП',
        docType: 'quotation',
        isDefault: false,
        backgroundImage: url,
  ...
  saveAsTemplate(): void {
    ...
    this.crudApi.create('/document-templates', template)
      .pipe(
        tap({
          next: (result: any) => {
