...
| `app-kp-search` | `kp-search.component.ts` | Поиск с иконкой и debounce |
| `app-kp-tag` | `kp-tag.component.ts` | Тэг/бейдж статуса |
| `app-kp-tab-group` | `kp-tab-group/` | Segmented control |
| `app-kp-paginator` | `kp-paginator.component.ts` | Пагинатор |
| `kp-sortable` | `kp-sortable/` | DnD списки |
| **`app-kp-document-block-editor`** | `kp-document-block-editor/` | **Визуальный A4-редактор: template + instance shell** |
...
**Отсутствуют (gap):** `kp-form-layout` (как компонент).
...
| **Custom hub/special** | **7** | `login-page`, `dashboard-page`, `not-found-page`, `documents-page`, `quotation-editor`, `attributes-editor` (embedded), `document-template-editor` |
...
**FreezeGuard risk:**
- ✅ **quotations** (wip) — можно менять; 🧊 **orders** (frozen) — после разморозки
...
**Файлы (wip-safe):** `quotations-page.*`, `tenders-page.*`, `shipments-page.*`, `work-orders-page.*`, `purchase-orders-page.*`, `attributes-editor.*`, `dashboard-page.*`, `documents-page.*`

**Файлы (требуют разморозки):** `orders-page.*`, `directories-page.*`, `modules-page.*`, `products-page.*`
...
- P1: quotations-page (wip); orders, directories, modules — после разморозки
...
|--------|--------|--------------|
| `document-template-editor` | **step2✅** | **На shared-канвасе (mode=template)** — A4, drag-drop, FAB, фон, плейсхолдеры |
| ui-kit | wip | ✅ P0 — primary target |
| quotations | wip | ✅ P1/P2; **Инкремент 1 ✅** — shell mode (A4-оболочка + фон на shared-канвасе) |
| orders | frozen | 🧊 P1/P2 после разморозки |
| directories-page | frozen | 🧊 P1 после разморозки |
| dashboard | wip | ✅ P1 (dashboard cards) |
| document-templates | wip | ✅ Шаг 2 — визуальный A4-редактор на shared-канвасе |
